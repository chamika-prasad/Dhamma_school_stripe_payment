        ...
        <!-- navbar 1-->
            ...
            <li class="nav-item">
              <a class="nav-link" href="#">
                ...
                font-size: 24px;
                line-height: 28px;
                color:white;">&ensp;Payment Details</span> </a>
            </li>
...
<!-- application -->
   ...
   <div class="form-outline mb-4" style="margin-top:1.5cm; padding-left:2cm; padding-right:2cm;">
    ...
    <form action="{{url('remainSearch')}}" method="POST">
      @csrf
    <div class="input-group">
      <div class="form-outline">
        <input type="search" name="search" id="form1" class="form-control" placeholder="Search"/>
      </div>
      <button type="submit" class="btn btn-dark" style="width: 1.5cm; height:1cm">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
          <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/>
        </svg>
      </button><a href="{{url('remaining')}}" style="padding-top:18px; padding-left:8px; text-decoration:none; color:black; ">Show all Payment</a>
    </div>
  </form><br>
  ...
  <!-- search bar-->
    ...
    <table class="table table-dark" >
        ...
        <tbody>
            @foreach ($remaings as $remaing)
          <tr>
            <td>{{$id++}}</td>
            <td>{{date('Y-m-d',strtotime($remaing->created_at))}}</td>
            <td>{{date('H:i:s',strtotime($remaing->created_at))}}</td>
            <td>{{$remaing->user_id}}</td>
            <td>{{$remaing->userName}}</td>
            <td>{{$remaing->total_amount}}</td>
            <td>{{$remaing->paid_amount}}</td>
            <td>{{$remaing->remaining_amount}}</td>
          </tr>
